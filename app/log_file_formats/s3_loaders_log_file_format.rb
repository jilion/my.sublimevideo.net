# encoding: utf-8
class S3LoadersLogFileFormat < RequestLogAnalyzer::FileFormat::Base
  extend RequestLogAnalyzer::FileFormat::CommonRegularExpressions
  extend AmazonLogFileFormat
  ...
  report do |analyze|
    analyze.traffic(:bytes_sent, title: :traffic_s3,
      category: lambda { |r| loader_token_from(r[:key]) },
      if: lambda { |r| loader_token?(r[:key]) && s3_get_request?(r[:operation]) }
    )
  ...
  class Request < RequestLogAnalyzer::Request
    ...
    def convert_timestamp(value, definition)
      "#{value[7,4]}#{AmazonLogFileFormat.months[value[3,3]]}#{value[0,2]}#{value[12,2]}#{value[15,2]}#{value[18,2]}".to_i
    end
