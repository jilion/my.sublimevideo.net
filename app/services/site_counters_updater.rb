class SiteCountersUpdater
  ...
  def self.update_last_30_days_counters_for_not_archived_sites
    Site.not_archived.find_each(batch_size: 100) do |site|
      updater = new(site)
      updater.update_last_30_days_video_tags_counters
      updater.update_last_30_days_video_views_counters
    end
  end
  ...
  def update_last_30_days_video_tags_counters
    count = VideoTag.count(_site_token: site.token, last_30_days_active: true)
    site.update_column(:last_30_days_video_tags, count)
  end
