...
module UserModules::Scope
  ...
  included do
    ...
    # billing
    scope :free, lambda {
      active.includes(:sites).where("(#{Site.active.paid_plan.select("COUNT(sites.id)").where("sites.user_id = users.id").to_sql}) = 0")
    }
    scope :paying, lambda {
      active.includes(:sites).merge(Site.active.paid_plan)
    }
