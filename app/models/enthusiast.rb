class Enthusiast < ActiveRecord::Base
  ...
  scope :by_email,              lambda { |way = 'desc'| order{ email.send(way) }.order{ created_at.desc } }
  scope :by_starred,            lambda { |way = 'asc'| order{ starred.send(way) }.order{ created_at.desc } }
  scope :by_confirmed,          lambda { |way = 'asc'| order{ confirmed_at.send(way) }.order{ created_at.desc } }
  scope :by_interested_in_beta, lambda { |way = 'desc'| order{ interested_in_beta.send(way) }.order{ created_at.desc } }
  scope :by_invited,            lambda { |way = 'desc'| order{ invited_at.send(way) }.order{ created_at.desc } }
