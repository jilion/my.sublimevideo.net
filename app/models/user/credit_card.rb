...
module User::CreditCard
  ...
  def credit_card?
    [cc_type, cc_last_digits, cc_expire_on].all?(&:present?)
  end
  alias :cc? :credit_card?

  def pending_credit_card?
    [pending_cc_type, pending_cc_last_digits, pending_cc_expire_on].all?(&:present?)
  end
  ...
  # before_save if credit_card.valid?
  ...
  def check_credit_card(options={})
    ...
    process_cc_authorize_and_save(authorize.params)
  end
