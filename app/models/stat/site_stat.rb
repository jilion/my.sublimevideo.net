module Stat::SiteStat
  ...
  included do
    ...
    scope "after".to_sym, lambda { |date| where(d: { "$gte" => date.to_i }).order_by([:d, :asc]) }
    scope "before".to_sym,  lambda { |date| where(d: { "$lte" => date.to_i }).order_by([:d, :asc]) }
    scope "between".to_sym, lambda { |start_date, end_date| where(d: { "$gte" => start_date.to_i, "$lte" => end_date.to_i }).order_by([:d, :asc]) }
  end
  ...
  def self.json(site_token, period = 'days')
    ...
    json_stats = if from.present? && to.present?
      case period
      when 'seconds'
        Stat::SiteSecondStat.last_stats(token: site_token, period: period, from: from, to: to, fill_missing_days: false, demo: demo)
      when 'minutes'
        Stat::SiteMinuteStat.last_stats(token: site_token, period: period, from: from, to: to, fill_missing_days: true, demo: demo)
      when 'hours'
        Stat::SiteHourStat.last_stats(token: site_token, period: period, from: from, to: to, fill_missing_days: true, demo: demo)
      when 'days'
        Stat::SiteDayStat.last_stats(token: site_token, period: period, from: from, to: to, fill_missing_days: true, demo: demo)
      end
    else
      []
    end
  ...
  def self.period_intervals(site_token, period)
    ...
    when 'minutes'
      ...
      if site.plan_stats_retention_days == 0
        ...
        last_minute_stat = Stat::SiteMinuteStat.order_by([:d, :asc]).last
    ...
    when 'days'
      site  = ::Site.find_by_token(site_token)
      stats = Stat::SiteDayStat.where(t: site_token).order_by([:d, :asc])
