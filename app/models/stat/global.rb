class Stat::Global
  ...
  # = Instance Methods =
  # ====================
  def self.usage(start_date, site_id = nil)
    conditions = { :day => { "$gte" => start_date.to_time.beginning_of_day.utc } }
    conditions[:site_id] = site_id if site_id
    ...
    SiteUsage.collection.group(
      ...
      [:day],
      conditions,
      { :main_usage => 0, :dev_usage => 0 }, # memo variable name and initial value
      "function(doc, prev) {
