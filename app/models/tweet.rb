...
class Tweet
  ...
  def self.save_new_tweets_and_sync_favorite_tweets
    ...
    KEYWORDS.each do |keyword|
      ...
      begin
        results = with_rescue_and_retry(5) { search.fetch }
      ...
      end while with_rescue_and_retry(5) { search.fetch_next_page }
    end
  ...
  def self.with_rescue_and_retry(times)
    rescue_and_retry(times, Errno::ETIMEDOUT, Errno::ECONNRESET, Twitter::BadGateway, Twitter::ServiceUnavailable) do
      yield
    end
  end
