  ...
  render: ->
    if MSVStats.stats.size() > 0
      new Highcharts.Chart
        ...
        tooltip:
          formatter: ->
            # "<b>#{@point.name}</b> #{@y} hits (#{(Highcharts.numberFormat @percentage, 1)} %)"
            "#{@y} hits (#{(Highcharts.numberFormat @percentage, 1)} %)"
        plotOptions:
          pie:
            animation: true
            showInLegend: true
            ...
            dataLabels:
              ...
              # formatter: ->
              #   '<b>' + @point.name + '</b>: ' + (Highcharts.numberFormat @percentage, 1) + ' %'
        series: [
          type: 'pie'
          name: 'Browser + OS'
