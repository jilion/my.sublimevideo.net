class MySublimeVideo.Helpers.HistoryHelper
  ...
  @currentUrlWithNewQuery: (key, value) ->
    ...
    "#{currentLocation.protocol}//#{currentLocation.hostname}#{currentLocation.pathname}#{currentSearch}"

  @updateQueryInUrl: (key, value) ->
    if history and history.pushState
      history.pushState({}, document.title, @currentUrlWithNewQuery(key, value))
