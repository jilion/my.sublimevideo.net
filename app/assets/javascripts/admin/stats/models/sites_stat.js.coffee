    ...
    while from <= to
      ...
      value = if stat?
        if selected.length > 1 # attribute is something like: ["pa", "premium"]
          ...
          switch selected[0]
            when 'all' then this.recursiveHashSum(stat.get('fr')) + this.recursiveHashSum(stat.get('sp')) + stat.get('tr') + this.recursiveHashSum(stat.get('pa')) + stat.get('su') + stat.get('ar')

            when 'active' then this.recursiveHashSum(stat.get('fr')) + this.recursiveHashSum(stat.get('sp')) + stat.get('tr') + this.recursiveHashSum(stat.get('pa'))
