module InvoicesHelper
  ...
  def charging_status(invoice)
    if invoice.open?
      ...
      "#{content_tag(:strong, "Charging failed")} on #{l(invoice.last_failed_at, :format => :minutes_timezone)} with the following error: #{content_tag(:em, "\"#{invoice.last_failed_transaction.error}\"")}".html_safe
    end
  end
