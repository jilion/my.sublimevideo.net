...
describe UrlsHelper do
  ...
  describe '.cdn_url' do
    context 'Rails.env == development' do
      ...
      it { helper.cdn_url('foo').should eq '//s3.amazonaws.com/dev.sublimevideo/foo' }
      it { helper.cdn_url('/foo').should eq '//s3.amazonaws.com/dev.sublimevideo/foo' }
      it { helper.cdn_url('foo/').should eq '//s3.amazonaws.com/dev.sublimevideo/foo/' }
      it { helper.cdn_url('/foo.js').should eq '//s3.amazonaws.com/dev.sublimevideo/foo.js' }
    end

    context 'Rails.env == staging' do
      before { Rails.stub(env: 'staging') }

      it { helper.cdn_url('foo').should eq '//cdn.sublimevideo.net-staging/foo' }
      it { helper.cdn_url('/foo').should eq '//cdn.sublimevideo.net-staging/foo' }
      it { helper.cdn_url('foo/').should eq '//cdn.sublimevideo.net-staging/foo/' }
      it { helper.cdn_url('/foo.js').should eq '//cdn.sublimevideo.net-staging/foo.js' }
    end
