...
describe OauthToken do
  ...
  context "Factory" do
    before(:all) { @token = FactoryGirl.create(:oauth_token) }
    subject { @token }
  ...
  describe "Scopes" do
    before(:all) do
      ...
      @new_token         = FactoryGirl.create(:oauth_token, user: @user, authorized_at: nil, invalidated_at: nil)
      @authorized_token  = FactoryGirl.create(:oauth_token, user: @user, authorized_at: Time.now.utc, invalidated_at: nil)
      @invalidated_token = FactoryGirl.create(:oauth_token, user: @user, authorized_at: Time.now.utc, invalidated_at: Time.now.utc)
    end
