...
describe Stats::SiteStatsStat do
  ...
  describe ".delay_create_stats" do
    it "should delay create_stats if not already delayed" do
      expect { described_class.delay_create_stats }.to change(Delayed::Job.where { handler =~ '%Stats::SiteStatsStat%create_stats%' }, :count).by(1)
    end

    it "should not delay create_stats if already delayed" do
      described_class.delay_create_stats
      expect { described_class.delay_create_stats }.to_not change(Delayed::Job.where{ handler =~ '%Stats::SiteStatsStat%create_stats%' }, :count)
    end
