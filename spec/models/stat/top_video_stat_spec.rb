    ...
    context "last 24 hours" do
      ...
      before(:each) do
        6.times do |video_i|
          ...
          24.times do |hour_i|
            next if hour_i%2 == 0
            Factory.create(:top_video_hour_stat, st: 'site1234', u: "video#{video_i}", d: (hour_i + 1).hours.ago.utc.change(min: 0),
              vv: 2 * (video_i + hour_i),
              vl: 2 * (video_i * hour_i)
            )
          end
      ...
      specify { Stat::TopVideoStat.top_videos('site1234', period: 'hours', from: @from, to: @to, count: 5)[:videos].should have(5).items }
      specify { Stat::TopVideoStat.top_videos('site1234', period: 'hours', from: @from, to: @to, count: 5)[:total].should eq 6 }
      specify { Stat::TopVideoStat.top_videos('site1234', period: 'hours', from: @from, to: @to, count: 5)[:from].should eq @from.to_i }

      it "adds video_tag meta data" do
        video = Stat::TopVideoStat.top_videos('site1234', period: 'hours', from: @from, to: @to, count: 5)[:videos].first
      ...
      before(:each) do
        6.times do |video_i|
          ...
          62.times do |second_i|
            ...
            Factory.create(:top_video_second_stat, st: 'site1234', u: "video#{video_i}", d: (second_i).seconds.ago.utc.change(usec: 0),
              vv: 2 * (video_i + second_i),
              vl: 2 * (video_i * second_i)
            )
          end
        end
      end

      specify { Stat::TopVideoStat.top_videos('site1234', period: 'seconds', from: @from, to: @to, count: 1)[:videos].should have(6).items }
      specify { Stat::TopVideoStat.top_videos('site1234', period: 'seconds', from: @from, to: @to, count: 5)[:total].should eq 6 }
      specify { Stat::TopVideoStat.top_videos('site1234', period: 'seconds', from: @from, to: @to, count: 5)[:from].should eq @from.to_i }

      it "replaces vv_hash by vv_array and vl_hash by vl_hash" do
        videos = Stat::TopVideoStat.top_videos('site1234', period: 'seconds', from: @from, to: @to, count: 5)[:videos].sort_by! { |video| video["n"] }.reverse
        video  = videos.first
