...
describe Stat::Video do
  ...
  describe ".top_videos" do
    ...
    context "last 24 hours" do
      ...
      specify { Stat::Video.top_videos('site1234', period: 'hours', count: 5)[:videos].should have(5).items }
      specify { Stat::Video.top_videos('site1234', period: 'hours', count: 5)[:total].should eq 6 }
      specify { Stat::Video.top_videos('site1234', period: 'hours', count: 5)[:from].should eq 24.hour.ago.utc.change(min: 0).to_i }
    ...
    context "last 61 seconds" do
      ...
      specify { Timecop.freeze @second do
        Stat::Video.top_videos('site1234', period: 'seconds', count: 1)[:videos].should have(6).items
      end }
      specify { Timecop.freeze @second do
        Stat::Video.top_videos('site1234', period: 'seconds', count: 5)[:total].should eq 6
      end }
      specify { Timecop.freeze @second do
        Stat::Video.top_videos('site1234', period: 'seconds', count: 5)[:from].should eq 61.seconds.ago.utc.change(usec: 0).to_i
      end }
