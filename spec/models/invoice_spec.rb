...
describe Invoice do
  ...
  describe "Scopes" do
    before(:all) do
      ...
      @unpaid_invoice = Factory(:invoice, state: 'unpaid')
      @failed_invoice = Factory(:invoice, state: 'failed')
      @paid_invoice   = Factory(:invoice, state: 'paid')
    end

    pending "#between" do
      specify { Invoice.between(24.hours.ago, 12.hours.ago).all.should == [@failed_invoice] }
    end
  ...
  describe "Class Methods" do
    ...
    describe ".renew_active_sites_and_create_invoices" do
      before(:all) do
        ...
        @plan3 = Factory(:plan, cycle: "year")

        # to be renewed
        @site1 = Factory(:site, plan: @plan1, paid_plan_cycle_ended_at: 1.day.ago.end_of_day)
        @site2 = Factory(:site, plan: @plan1, next_cycle_plan: @plan2, paid_plan_cycle_ended_at: 1.day.ago.end_of_day)
        # not to be renewed
        @site3 = Factory(:site, plan: @plan1, paid_plan_cycle_ended_at: 1.day.from_now.end_of_day)
        @site4 = Factory(:site, plan: @plan1, next_cycle_plan: @plan2, paid_plan_cycle_ended_at: 1.day.from_now.end_of_day)
      end
      before(:each) do
      ...
      it "should update site that need to be renewed" do
        @site1.reload.paid_plan_cycle_ended_at.to_i.should_not == 1.day.ago.end_of_day.to_i
        @site2.reload.paid_plan_cycle_ended_at.to_i.should_not == 1.day.ago.end_of_day.to_i
      end

      it "should not update site that don't need to be renewed" do
        @site3.reload.paid_plan_cycle_ended_at.to_i.should == 1.day.from_now.end_of_day.to_i
        @site4.reload.paid_plan_cycle_ended_at.to_i.should == 1.day.from_now.end_of_day.to_i
      end

      it "should delay create invoice for site that need to be renewed" do
        ...
        @site1.invoices.size.should == 1
        @site2.invoices.size.should == 1
        @site3.invoices.size.should == 0
        @site4.invoices.size.should == 0
      end
    end
    ...
    describe ".build" do
      before(:all) do
        ...
        Timecop.travel(Time.utc(2010,2).beginning_of_month) do
          @site = Factory(:site, :user => @user, :plan => @plan1, :activated_at => Time.now)
        end
      end
      before(:each) do
      ...
      context "site plan has changed between invoice.ended_at and Time.now (site used in the invoice should be the one at the moment at the end of the period given to build)" do
        before(:all) do
          ...
          Timecop.travel(Time.utc(2010,3,2)) do
            with_versioning { @site.reload.update_attributes(:plan_id => @plan2.id) }
          end
        end
