...
describe SiteUsage do
  ...
  describe "with cdn.sublimevideo.net.log.1286528280-1286528340.gz logs file" do
    before(:each) do
      ...
      @log = FactoryGirl.create(:log_voxcast, :name => 'cdn.sublimevideo.net.log.1286528280-1286528340.gz')
      ...
      @site2 = FactoryGirl.create(:site, :user => @site1.user, :hostname => 'sonymusic.se').tap { |s| s.token = 'mhud9lff'; s.save }
    end
  ...
  describe "with 4076.voxcdn.com.log.1308045840-1308045900.gz logs file" do
    before(:each) do
      ...
      @log = FactoryGirl.create(:log_voxcast, :name => '4076.voxcdn.com.log.1308045840-1308045900.gz')
      @trackers = LogAnalyzer.parse(@log.file, 'LogsFileFormat::VoxcastSites')

      @site1 = FactoryGirl.create(:site, :hostname => 'customerhub.net', wildcard: true).tap { |s| s.token = '9pfe3uop'; s.save }
      @site2 = FactoryGirl.create(:site, :user => @site1.user, :hostname => 'farmerswifeplay.com').tap { |s| s.token = '87r9xy5e'; s.save }
    end
  ...
  describe "Trackers parsing with voxcast cdn.sublimevideo.net.log.1275002700-1275002760.gz logs file" do
    before(:each) do
      ...
      @site1 = FactoryGirl.create(:site, :hostname => 'zeno.name').tap { |s| s.token = 'g3325oz4'; s.save }
      @site2 = FactoryGirl.create(:site, :user => @site1.user, :hostname => 'octavez.com').tap { |s| s.token = 'g8thugh6'; s.save }

      @log = FactoryGirl.create(:log_voxcast)
      @trackers = LogAnalyzer.parse(@log.file, 'LogsFileFormat::VoxcastSites')
  ...
  describe "Trackers parsing with s3 loaders" do
    before(:each) do
      @site1 = FactoryGirl.create(:site).tap { |s| s.token = 'gperx9p4'; s.save }
      @site2 = FactoryGirl.create(:site, :user => @site1.user, :hostname => 'google.com').tap { |s| s.token = 'pbgopxwy'; s.save }

      @log = FactoryGirl.create(:log_s3_loaders)
