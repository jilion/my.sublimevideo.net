    ...
    describe ".send_usage_alerts" do
      ...
      context "with site's usage more than plan's limit and more than next plan price" do
        ...
        context "with plan_player_hits_reached alert sent before this month and next_plan_recommended alert never sent" do
          before(:all) { @site = Factory(:site, :plan => @plan1, :activated_at => 2.months.ago, :plan_player_hits_reached_alert_sent_at => 1.month.ago, :next_plan_recommended_alert_sent_at => nil) }
          subject { @site }
        ...
        context "with plan_player_hits_reached alert and next_plan_recommended alert sent before this month" do
          before(:all) { @site = Factory(:site, :plan => @plan1, :activated_at => 2.months.ago, :plan_player_hits_reached_alert_sent_at => 1.month.ago, :next_plan_recommended_alert_sent_at => 1.month.ago) }
          subject { @site }
        ...
        context "with plan_player_hits_reached alert already sent during the month and next_plan_recommended alert not sent" do
          before(:all) { @site = Factory(:site, :plan => @plan1, :activated_at => 2.months.ago, :plan_player_hits_reached_alert_sent_at => Time.now.utc.beginning_of_month, :next_plan_recommended_alert_sent_at => nil) }
          subject { @site }
