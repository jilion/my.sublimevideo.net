...
class AwesomeVersionClass < Struct.new(:version)
...
describe Service::SemanticVersioning do
  ...
  describe "#version" do
    # Valid
    {
      ...
      it "'#{input}' gives '#{result}'" do
        AwesomeVersionClass.new(input).version.should eq(result)
      end
    end
    ...
    [
      ...
      it "'#{invalid_version}' is invalid" do
        expect { AwesomeVersionClass.new(invalid_version) }.to raise_error(InvalidVersion)
      end
    end
  end
      ...
      context "with #{release} release" do
        hash.each do |version, result|
          it "increments '#{version}' to '#{result}'" do
            v = AwesomeVersionClass.new(version)
  ...
  describe "comparaison" do
    it "compares with simple version" do
      v200 = AwesomeVersionClass.new('2.0.0')
      v124 = AwesomeVersionClass.new('1.2.4')
      v123 = AwesomeVersionClass.new('1.2.3')
      v130 = AwesomeVersionClass.new('1.3.0')
    ...
    it "compares with complex version" do
      v100_aplha1 = AwesomeVersionClass.new('1.0.0-alpha.1')
      v100_beta2 = AwesomeVersionClass.new('1.0.0-beta.2')
      v100 = AwesomeVersionClass.new('1.0.0')
      vinf = AwesomeVersionClass.new('*')
  ...
  describe "ranges" do
    {
      ...
      it "'#{version}' is between #{range}" do
        AwesomeVersionClass.new(version).between?(
          AwesomeVersionClass.new(range.first),
          AwesomeVersionClass.new(range.last)
        ).should be_true
      end
    end
  ...
  describe ".dependency_range" do
    {
      ...
      it "'#{identifier}' dependency syntax gives #{exculsive_range} range (exclusive)" do
        dependency_range = AwesomeVersionClass.dependency_range(identifier)
