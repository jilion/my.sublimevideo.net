...
describe HostnameUniquenessValidator do
  ...
  context "on create" do
    it "should scope by user" do
      site2 = FactoryGirl.build(:new_site, :user => site.user)
    ...
    it "should not add an error if the hostname is blank" do
      site2 = FactoryGirl.build(:new_site)
    ...
    it "should allow 2 users to register the same hostname" do
      site2 = FactoryGirl.build(:new_site, :user => FactoryGirl.create(:user))
      validate_hostname_uniqueness(site2, :hostname, site.hostname)
      site2.errors[:hostname].size.should == 0
    end
  ...
  context "on update" do
    subject { FactoryGirl.create(:site, :user => site.user).tap { |s| s.update_attribute(:cdn_up_to_date, true) } }
    ...
    it "should allow 2 users to register the same hostname" do
      site2 = FactoryGirl.create(:site, :user => FactoryGirl.create(:user))
      validate_hostname_uniqueness(site2, :hostname, site.hostname)
      site2.errors[:hostname].size.should == 0
    end
