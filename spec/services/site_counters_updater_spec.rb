...
describe SiteCountersUpdater do
  ...
  before { allow(AddonPlan).to receive(:get) }
  ...
  describe '.update_not_archived_sites' do
    it 'calls #update and #update_not_archived_sites on each non-archived sites' do
      ...
      expect(described_class).to delay(:update, queue: 'my-low').with(site.id)
  ...
  describe '#update' do
    before {
      allow(SiteAdminStat).to receive(:last_days_starts) { [1,2,3] }
      allow(SiteStat).to receive(:last_days_starts) { [1,2,3] }
      allow(VideoTag).to receive(:count) { 0 }
    }
    ...
    context "with site.first_admin_starts_on unset" do
      before { allow(site).to receive(:first_admin_starts_on) { nil } }

      context "with some last_30_days_admin_starts" do
        before { allow(site).to receive(:last_30_days_admin_starts) { 1 } }
      ...
      context "with no last_30_days_admin_starts" do
        before { allow(site).to receive(:last_30_days_admin_starts) { 0 } }
    ...
    context "with site.first_admin_starts_on set" do
      before { allow(site).to receive(:first_admin_starts_on) { Date.yesterday } }
    ...
    context "with some last_30_days_admin_starts" do
      ...
      context "with realtime stats addon" do
        before { allow(site).to receive(:subscribed_to?) { true } }
      ...
      context "without realtime stats addon" do
        before { allow(site).to receive(:subscribed_to?) { false } }
