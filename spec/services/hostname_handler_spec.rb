...
describe HostnameHandler do
  ...
  describe "include?" do
    it { subject.include_hostname?("http://localhost:3000, localhost", 'localhost').should be_true }
    it { subject.include_hostname?("124.123.151.123, localhost", '124.123.151.123').should be_true }
    it { subject.include_hostname?("127.0.0.1, bob, 127.0.0.1", 'bob').should be_true }
    it { subject.include_hostname?("*.*, *.*", '*.*').should be_true }
    it { subject.include_hostname?("google.fr, jilion.com", "google.fr").should be_true }

    it { subject.include_hostname?(nil, 'jilion.com').should be_false }
    it { subject.include_hostname?('jilion.com', nil).should be_false }
    it { subject.include_hostname?('jilion.com', "").should be_false }
    it { subject.include_hostname?(nil, nil).should be_false }
    it { subject.include_hostname?("", "").should be_false }
    it { subject.include_hostname?("", 'jilion.com').should be_false }
    it { subject.include_hostname?("localhost, jilion", 'jilion.com').should be_false }
  end
